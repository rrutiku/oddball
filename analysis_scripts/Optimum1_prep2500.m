...
CFGs{end}.trialdef.prestim    = 0.3;          % in seconds; includes space for edge artifacts (x2 due to resampling ect)
CFGs{end}.trialdef.poststim   = 0.6;          % in seconds; includes space for edge artifacts
...
difts = diff(CFGs{end}.trl(:,1)) / 2500; % should be around 0.55
...
raw_data = ft_preprocessing(CFGs{end});  % load the raw data
...
% NOISE_manual = cfg.artfctdef.visual.artifact;
...
CFGs{end+1} = [];
CFGs{end}.toilim = [-0.1 0.4];
raw_data  = ft_redefinetrial(CFGs{end}, raw_data);

% Change sampleinfo to avoid repeating sample numbers
raw_data.sampleinfo = [1000:2000:2000*length(trl_raw)]';
...
badchan_2    = badchan;
allbadchan_2 = {}; try allbadchan_2 = allbadchan{end}; catch; end
