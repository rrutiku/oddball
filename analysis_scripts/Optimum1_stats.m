...
%     ft_multiplotER(cfg, all_stand_avg, all_dev_avg);
    ...
    aMMN_stat = ft_timelockstatistics(cfg, deviants_wt, standards_wt);
    save([PATH, 'pp_', num2str(s), '\\aMMN_stat'], 'aMMN_stat', 'cfg'); 
    %load([PATH, SUBJECT, 'aMMN_stat']);
    
    figure
    subplot_tight(1,2,1);
    contourf(aMMN_stat.mask)
    subplot_tight(1,2,2);
    contourf(aMMN_stat.negclusterslabelmat==1)
